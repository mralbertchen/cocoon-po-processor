...
app.post("/submit", function(request, response) {
  ...
  POExists(request.body.PO_Num, function(POid) {
    // check if PO exists
    if (POid) {
      // if exists
...
function createPO(arrReq, clientID, callback) {
  ...
  base("PO List").create(
    ...
    function(err, record) {
      ...
      var tempReq = {
        // create a temporary request array
        Item: [],
        Price: [],
        Desc: [],
        Qty: []
      ...
      var i = 0;
      var loopArray = function(reqBod) {
        // create asynchronous loop
          ...
          base("PO Items").create(
            ...
            function(err, record) {
              ...
              console.log(
                "Created record",
                record.id,
                record.fields["Unit Price"],
                record.fields["Total Units"]
              );
            ...
            function page(records, fetchNextPage) {
              ...
              records.forEach(function(record) {
                ...
                if (thisID == deleteID) {
                  ...
                  base("PO Items").destroy(record.id, function(
                    err,
                    deletedRecord
                  ) {
                    if (err) {
